...
ERRORS = {}
CONFIG = sublime.load_settings("xml-validation.sublime-settings")
...
def find_exec(lint_exec):
    ...
    for path in sys.path:
        print path
        if path == env_path:
            env_path_already_there = True
    if env_path_already_there == False:
...
def format(view):
    ...
    stdout, stderr, returncode = cmd(COMMAND_BASE % (get_exec_xmllint(), '--format', '"'+view.file_name()+'"'))
...
def validate_dtd(view):
    ...
    stdout, stderr, returncode = cmd(COMMAND_BASE % (get_exec_xmllint(), '--noout --valid', '"'+view.file_name()+'"'))
...
def is_xml_file(view):
    if view.file_name().endswith('.xml'):
        return True
    elif view.file_name().endswith('.scml'):
        return True
    else:
        return False
